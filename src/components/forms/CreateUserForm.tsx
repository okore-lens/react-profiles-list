...
const CreateUserForm = ({ closeModal }: { closeModal: () => void }) => {
	...
	const onSubmit: SubmitHandler<userData> = async (data) => {
		try {
			await appCtx?.addUser(data);
			setSuccess(true);
			await appCtx?.updateUsersList([
				{
					...data,
					id: (Math.floor(Math.random() * (20 - 11 + 1)) + 11).toLocaleString(),
				},
				...appCtx.users,
			]);
	...
	return (
		<section>
			{success && (
				<RequestStatus type="success" message="User Created Successfully" />
			)}
			{!!errors.afterSubmit && (
